...
public class productService {
    ...
    public List<productDTO> getFilteredProducts(
        ...
        if(minPopularity != null && maxPopularity != null){
            ...
            filtered = filtered.stream()
                    .filter(p -> p.getPopularityScore().compareTo(minPopularity.multiply(BigDecimal.valueOf(1))) >= 0)
                    .collect(Collectors.toList());
        } else if (maxPopularity != null) {
            filtered = filtered.stream()
                    .filter(p -> p.getPopularityScore().compareTo(maxPopularity.multiply(BigDecimal.valueOf(1))) <= 0)
                    .collect(Collectors.toList());
        }
